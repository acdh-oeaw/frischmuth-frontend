<script lang="ts" setup>
import { Card } from "../components/ui/card";
</script>

<template>
	<MainContent class="container py-8">
		<div class="mx-auto flex size-full grid-cols-3 items-center gap-4 font-semibold">
			<div class="mx-auto grid max-w-7xl gap-4 font-bold grid-fluid-cols-60">
				<Card class="relative aspect-square overflow-hidden bg-frisch-orange p-4 text-white">
					<span>ORTE</span>
				</Card>
				<Card class="relative aspect-square overflow-hidden bg-frisch-grey p-4 text-frisch-indigo">
					<span class="relative">Istanbul</span>
				</Card>
				<Card
					class="relative aspect-square overflow-hidden bg-frisch-orange-light p-4 text-frisch-orange"
				>
					<span>"Und dann fiel ich aus den Wolken"</span>
				</Card>
				<Card class="relative aspect-square overflow-hidden bg-frisch-orange p-4 text-white">
					<span>FORSCHUNGS-HINSICHTEN</span>
				</Card>
				<Card class="relative aspect-square overflow-hidden p-4">
					<img
						alt=""
						class="absolute inset-0 size-full object-cover"
						src="/assets/images/writing_placeholder.png"
					/>
				</Card>
				<Card class="relative aspect-square overflow-hidden bg-frisch-grey p-4 text-frisch-indigo">
					<span>Aus dem Archiv</span>
				</Card>
				<Card class="relative aspect-square overflow-hidden bg-frisch-orange p-4 text-white">
					<span>NAMEN</span>
				</Card>
				<Card class="relative aspect-square overflow-hidden bg-frisch-indigo p-4 text-white">
					<span>JOURNALISTISCHE REZEPTION</span>
				</Card>
				<Card
					class="relative aspect-square overflow-hidden bg-frisch-orange-light p-4 text-frisch-orange"
				>
					<span>
						"Keine Grenze kann so dicht geschlossen werden, dass sie nicht von denen, die unbedingt
						ihr Glück versuchen wollen, überwunden wird.
					</span>
				</Card>
				<Card class="relative aspect-square overflow-hidden bg-frisch-orange p-4 text-white">
					<span>GLOSSAR</span>
				</Card>
				<Card class="relative aspect-square overflow-hidden bg-frisch-indigo p-4 text-white">
					<span>AKADEMISCHE REZEPTION</span>
				</Card>
				<Card
					class="relative aspect-square overflow-hidden bg-frisch-orange-light p-4 text-frisch-orange"
				>
					<span>
						"Für die meisten Schriftsteller gibt es ohnehin nur eine wirkliche Heimat, und das ist
						die Sprache, in der sie schreiben."
					</span>
				</Card>
				<Card class="relative aspect-square overflow-hidden bg-frisch-grey p-4 text-frisch-indigo">
					<span>Postkoloniale Theorie</span>
				</Card>
				<Card class="relative aspect-square overflow-hidden bg-frisch-orange p-4 text-white">
					<span>INTERPRETATEME</span>
				</Card>
				<Card class="relative aspect-square overflow-hidden bg-frisch-grey p-4 text-frisch-indigo">
					<span>Kasanapodole</span>
				</Card>
			</div>
		</div>
	</MainContent>
</template>
